...
## Large files (Zenodo-only)

To keep the git repository lightweight, the following large derived tables are **not tracked in
git** (see `.gitignore`). They are included in the FAIR Zenodo bundle accompanying this manuscript
and are regenerated deterministically by the pipeline scripts noted below:

| File | Size | Regenerated by |
|------|------|----------------|
| `results/cellchat/lr_pair_strength_per_sample.csv` | ~59 MB | `code/12_lr_pair_specificity.py` |
| `results/deconvolution/pseudo_celltype_expr.csv` | ~15 MB | `code/04_deconvolve_bulk.py` |
| `results/deconvolution/pseudo_celltype_expr_microarray.csv` | ~10 MB | `code/12_lr_pair_specificity.py` |
| `results/trajectories/decoder_trajectories.csv` | ~8 MB | `code/05_gp_autoencoder.py` |

The raw OSD-782 count matrix (`data/osd782_normalized_counts.csv`) is likewise untracked and
re-downloaded from OSDR by `code/15_osd782_validation.py`.
